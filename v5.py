...
class SmartWeighingScale:
    ...
    def check_license(self):
        print("Entering check_license")  # Debug
        self.cursor.execute("SELECT license_key, expiry_date FROM license_keys ORDER BY id DESC LIMIT 1")
        result = self.cursor.fetchone()
        print(f"Retrieved license: {result}")  # Debug
        if result:
            stored_license_key, expiry_date_str = result
            print(f"Processing license key: {stored_license_key}, expiry: {expiry_date_str}")  # Debug
            try:
                expiry_date = datetime.strptime(expiry_date_str, "%Y-%m-%d").date()
                current_date = datetime.now().date()
                print(f"Current date: {current_date}, Expiry date: {expiry_date}")  # Debug
                print(f"Is current_date > expiry_date? {current_date > expiry_date}")  # Debug
                if current_date > expiry_date:
                    print("License expired, hiding main window and showing prompt")  # Debug
                    self.master.withdraw()
                    self.prompt_for_license_key()
                else:
                    print("License valid, creating main window")  # Debug
                    self.create_main_window()
            except Exception as e:
                print(f"Error in check_license: {str(e)}")  # Debug
                self.master.withdraw()
                self.prompt_for_license_key()
        else:
            print("No license key found, showing prompt")  # Debug
            self.master.withdraw()
            self.prompt_for_license_key()

    def prompt_for_license_key(self):
        print("Entering prompt_for_license_key")  # Debug
        self.license_window = tk.Toplevel(self.master)
        print("Toplevel window created")  # Debug
        self.license_window.title("License Key Required")
        self.license_window.geometry("450x250")
        self.license_window.configure(bg=BACKGROUND_COLOR)
        self.license_window.protocol("WM_DELETE_WINDOW", self.on_license_window_close)
        self.license_window.resizable(False, False)
        self.license_window.transient(self.master)
        self.license_window.grab_set()
        print("License window configured")  # Debug

        main_frame = tk.Frame(self.license_window, bg=BACKGROUND_COLOR)
        ...
        message_label = tk.Label(main_frame, 
                                text="Your license has expired. Please enter a new license key to continue:",
                                font=("Helvetica", 11), 
                                bg=BACKGROUND_COLOR, fg=TEXT_COLOR,
        ...
        self.license_window.bind('<Return>', lambda event: self.verify_and_update_license())
        print("License prompt fully set up")  # Debug
    ...
    def verify_and_update_license(self):
        ...
        # Basic validation - check if key has minimum length
        ...
        try:
            ...
            self.cursor.execute("DELETE FROM license_keys")  # Remove old keys
            # Set expiry date to 1 year from now for new license keys
            new_expiry = (datetime.now().date() + timedelta(days=365)).strftime("%Y-%m-%d")
            self.cursor.execute("INSERT INTO license_keys (license_key, expiry_date) VALUES (?, ?)",
                               (entered_key, new_expiry))
            self.conn.commit()
            ...
            # Show the main window and continue normal operation
    ...
    def on_license_window_close(self):
        # If the license window is closed without entering a key, exit the application
        result = messagebox.askyesno("Exit Application", 
                                   "Without a valid license key, the application cannot continue.\n\n" +
                                   "Do you want to exit the application?")
        if result:
            self.master.destroy()
        # If user chooses not to exit, keep the license window open
    ...
    def create_database(self):
        ...
        if self.cursor.fetchone()[0] == 0:
            # License key valid until Aug 2, 2025
            self.cursor.execute("INSERT INTO license_keys (license_key, expiry_date) VALUES (?, ?)",
                                ("LICENSE_KEY_BEFORE_AUG_2025", "2025-08-02"))
            # License key valid after Aug 2, 2025
            self.cursor.execute("INSERT INTO license_keys (license_key, expiry_date) VALUES (?, ?)",
    ...
    def setup_main_tab(self):
        tk.Label(self.tab_main, text="Weight Reading", font=("Helvetica", 20, "bold"), bg=BACKGROUND_COLOR, fg=TEXT_COLOR).pack(pady=20)

        self.weight_var = tk.StringVar(value="0.0000 g")
        tk.Label(self.tab_main, textvariable=self.weight_var, font=("Helvetica", 24, "bold"), bg="white",
                 fg=TEXT_COLOR, relief="solid", bd=2, width=15).pack(pady=10)

        self.result_label = tk.Label(self.tab_main, text="", font=("Helvetica", 16, "bold"), width=12, height=2)
        self.result_label.pack(pady=10)

        self.category_var = tk.StringVar()
        self.category_dropdown = ttk.Combobox(self.tab_main, textvariable=self.category_var,
                                              values=self.get_categories(), state="readonly",
                                              font=("Helvetica", 12))
